...
for nC = 1:size(mancovanInfo.userInput.compFiles, 1)
    
    tmp = icatb_spm_read_vols(icatb_spm_vol(mancovanInfo.userInput.compFiles(nC, :)));
    ...
    if (nC == 1)
        compData = zeros(size(tmp, 1), size(tmp, 2), mancovanInfo.userInput.numICs);
    end
...
compGroupNames = '';
covNames = '';
allFeatures = {'Spatial Maps', 'Timecourses Spectra', 'FNC Correlations'};
...
mancovanInfo = get_info_controls(handles);
if (~isfield(mancovanInfo.userInput, 'feature_params'))
    feature_params = icatb_mancovan_feature_options('tr', mancovanInfo.userInput.TR, 'mask_dims', mancovanInfo.userInput.HInfo(1).dim(1:3));
else
