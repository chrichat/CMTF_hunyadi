    ...
    for nF = 1:length(features)
        ...
        elseif (strcmpi(features{nF}, 'timecourses spectra'))
            ...
            for ncomps = comp_inds
                ...
                timecourses = icatb_loadComp(sesInfo, ncomps, 'vars_to_load', 'tc', 'subjects', mancovanInfo.good_sub_inds, 'average_runs', 1, ...
            ...
            timecourses = icatb_loadComp(sesInfo, comp_inds, 'vars_to_load', 'tc', 'subjects', mancovanInfo.good_sub_inds, 'average_runs', 1, ...
            ...
            for nSub = 1:mancovanInfo.numOfSub
                for nComp = 1:length(comp_inds)
                    if (strcmpi(feature_params.final.fnc_tc_despike, 'yes'))
                        timecourses(nSub, :, nComp) = despike_TC(timecourses(nSub, :, nComp), TR);
                    end
                    if (strcmpi(feature_params.final.fnc_tc_filter, 'yes'))
                        timecourses(nSub, :, nComp) = filt_data(timecourses(nSub, :, nComp), TR);
                    end
                end
                c = icatb_corr(squeeze(timecourses(nSub, :, :)));
                c(1:size(c, 1) + 1:end) = 0;
                c = mat2vec(icatb_r_to_z(c));
                if (nSub == 1)
                    fnc_corrs = zeros(mancovanInfo.numOfSub, numel(c));
                end
                fnc_corrs(nSub, :) = c(:)';
            end
            ...
            icatb_save(fullfile(mancovanInfo.outputDir, resultsFile), 'comp_est', 'comp_number', 'fnc_corrs');
