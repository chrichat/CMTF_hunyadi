...
% if (~isfield(mancovanInfo, 'resultsFile'))
%     error('Run the Mancovan analysis to visualize the results');
% end
...
promptPos =  [xOffset, yPos - popupHeight - yOffset - promptHeight, promptWidth, promptHeight];
...
popupPos(1) = popupPos(1) + popupPos(3) + xOffset;
popupPos(3) = 0.15;
popupH = icatb_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'popup', 'position', popupPos, 'string', char('fdr', 'none'), 'tag', 'threshdesc', 'enable', 'off');

handles_data.mancovan = mancovanInfo;
handles_data.structFile = fullfile(fileparts(which('gift.m')), 'icatb_templates', 'nsingle_subj_T1_2_2_5.nii');
...
try
    ...
    if (~isempty(findstr(lower(selListVal), 'features')))
        ...
        if (~isempty(fnc_inds))
            ...
            M = icatb_vec2mat(icatb_z_to_r(squeeze(mean(fnc_corrs))));
            M(isnan(M)) = 0;
            M = M + M';
            CLIM = max(abs(M(:)));
