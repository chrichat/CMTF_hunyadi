...
if (~isempty(maskFile) && strcmpi(modalityType, 'fmri'))
...
if ((numOfSub*numOfSess < 4) && (numOfSub*numOfSess > 1))
    numReductionSteps = 2;
end

if ((numOfSub*numOfSess > 4) && (numReductionSteps == 1))
    numReductionSteps = 2;
end
...
if (doEstimation == 1)
    ...
    if (~isfield(estimation_opts, 'PC3'))
    ...
    if (numReductionSteps == 3)
        numPC3 = round(eval([lower(estimation_opts.PC3), '(estimated_comps);']));
    end
...
sesInfo.userInput.numComp = inputData.numOfPC1;
...
elseif (numReductionSteps == 3)
    sesInfo.userInput.numOfPC2 = inputData.numOfPC2;
    sesInfo.userInput.numOfPC3 = inputData.numOfPC3;
    sesInfo.userInput.numComp = inputData.numOfPC3;
    numOfGroups2 = ceil(numOfDataSets/4);
    sesInfo.userInput.numOfGroups2 = numOfGroups2;
    sesInfo.userInput.numOfGroups3 = 1;
end
...
elseif strcmpi(ica_algo{algoType}, 'constrained ica (spatial)')
...
spatial_references = str2mat(spatial_references);
