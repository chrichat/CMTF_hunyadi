...
if (returnInterpdata)
    composite_data = getCompositeData(structData, data, returnValue, minInterval, maxInterval);
...
for ncoords = 1:size(coords, 1)
    ...
    if (ncoords == size(coords, 1))
        ...
        pos = [firstPos(1) + width/2 - colorBarWidth/2, lastPos(2) - 0.05, colorBarWidth, colorBarHeight];
        ch = colorbar('horiz');
...
maxInterval = maxInterval - unitColor;
